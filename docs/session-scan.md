...
- **`src/pi/summarize.ts`** — `createModelSummarizer`: the shared session-model wiring
  ...
  over it.
- **`src/pi/sessionScan.ts`** — the session prompt (outcomes plus 2–4 reusable technical
  takeaways), `scanPiSessions` (roots + tunables → `runSessionScan`), and the result formatter.
